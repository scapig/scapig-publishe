...
class RamlRepository @Inject()(val reactiveMongoApi: ReactiveMongoApi)  {
  ...
  def save(context: String, version: String, raml: String): Future[HasSucceeded] = {
    ...
    repository flatMap {  collection =>
      collection.update(BSONDocument("context" -> context, "version" -> version), ramlDocument, upsert = true) map(_ => HasSucceeded)
    }
  }
